        ...
        if (source.equals(target)) {
            ...
            try {
                ...
                if (!source.equals("BRL")) {
                    BCBResponse response = bcbClient.getBCBCurrency(source, "05-15-2026");
                    if (response.value().isEmpty())
                        throw new Exception("Currency not found for" + source);

                    sourceRate = Double.valueOf(
                            response.value().get(0).cotacaoVenda()
                    );
                }

                if (!target.equals("BRL")) {
                    BCBResponse response = bcbClient.getBCBCurrency(target, "05-15-2026");
                    if (response.value().isEmpty())
                        throw new Exception("Currency not found for" + target);

                    targetRate = Double.valueOf(
                            response.value().get(0).cotacaoVenda()
                    );
                }
